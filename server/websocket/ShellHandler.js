        ...
        ws.on('message', async (message) => {
            try {
                ...
                if (data.type === 'init') {
                    ...
                    // Session IDs starting with "new-" are freshly generated by the UI's
                    // "New Session" button — they should NOT be resumed via --resume.
                    const isNewSession = typeof sessionId === 'string' && sessionId.startsWith('new-');
                    const hasSession = isNewSession ? false : data.hasSession;
