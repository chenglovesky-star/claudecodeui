package com.iflytek.bigdata.flint.common.date;
...
public class DateUtil {
    private static final Pattern VARIABLE_PATTERN = Pattern.compile("^过去([0-9]+)天$");

    private static final Pattern VARIABLE_PATTERN2 = Pattern.compile("^最近([0-9]+)天$");
    ...
    public static final Pattern V_PATTERN = Pattern.compile("\\$\\{(.+?)#(.+?)\\}");

    public static final Pattern D_PATTERN = Pattern.compile("(\\+|\\-)?([0-9]+)[ ].*(month|day|hour|minute)s?");

    public static String replaceFormat(String content) {
        Matcher matcher = V_PATTERN.matcher(content);
        while (matcher.find()) {
            String timeModule = matcher.group(1);
            String format = matcher.group(2);
            Matcher m = D_PATTERN.matcher(timeModule);
            Date date = new Date();
            while (m.find()) {
                String symbol = m.group(1);
                String number = m.group(2);
                String time = m.group(3);
                Integer diffNumber = Integer.parseInt(number);
                if (StringUtils.isNotEmpty(symbol) && "-".equals(symbol)) {
                    diffNumber = -diffNumber;
                }
                switch (time) {
                    case "month":
                        date = DateUtils.addMonths(date, diffNumber);
                        break;
                    case "day":
                        date = DateUtils.addDays(date, diffNumber);
                        break;
                    case "hour":
                        date = DateUtils.addHours(date, diffNumber);
                        break;
                    case "minute":
                        date = DateUtils.addMinutes(date, diffNumber);
                        break;
                }
            }
            String d = DateFormatUtils.format(date, format);
            content = content.replace(matcher.group(0), d);
        }
        return content;
    }

    public static List<String> getDynamicDate(String timeInfo) {
        List<String> res = new ArrayList<>();
        String begin = "";
        String end = "";
        Matcher matcher = VARIABLE_PATTERN.matcher(timeInfo);
        Matcher matcher2 = VARIABLE_PATTERN2.matcher(timeInfo);
        if (matcher.find()) {
            Integer day = Integer.valueOf(matcher.group(1));
            begin = DateUtil.DateToString(DateUtil.addDay(new Date(), -day), DateStyle.YYYY_MM_DD);
            end = DateUtil.DateToString(DateUtil.addDay(new Date(), -1), DateStyle.YYYY_MM_DD);
        } else {

            if (matcher2.find()) {
                Integer day = Integer.valueOf(matcher.group(1));
                begin = DateUtil.DateToString(DateUtil.addDay(new Date(), -day), DateStyle.YYYY_MM_DD);
                end = DateUtil.DateToString(new Date(), DateStyle.YYYY_MM_DD);
            } else {
                switch (timeInfo) {
                    case "昨日":
                        begin = DateUtil.DateToString(DateUtil.addDay(new Date(), -1), DateStyle.YYYY_MM_DD);
                        end = DateUtil.DateToString(DateUtil.addDay(new Date(), -1), DateStyle.YYYY_MM_DD);
                        break;
                    case "今日":
                        begin = DateUtil.DateToString(new Date(), DateStyle.YYYY_MM_DD);
                        end = DateUtil.DateToString(new Date(), DateStyle.YYYY_MM_DD);
                        break;
                    case "上周":
                        begin = DateUtil
                                .DateToString(DateUtil.geLastWeekMonday(new Date()), DateStyle.YYYY_MM_DD);
                        end = DateUtil.DateToString(DateUtil.addDay(DateUtil.getThisWeekMonday(new Date()), -1),
                                DateStyle.YYYY_MM_DD);
                        break;
                    case "本周":
                        begin = DateUtil
                                .DateToString(DateUtil.getThisWeekMonday(new Date()), DateStyle.YYYY_MM_DD);
                        end = DateUtil.DateToString(DateUtil.addDay(new Date(), -1), DateStyle.YYYY_MM_DD);
                        break;
                    case "上月":
                        begin = DateUtil.DateToString(DateUtil.addMonth(DateUtil.getCurrentMonthFirstDay(), -1),
                                DateStyle.YYYY_MM_DD);
                        end = DateUtil.DateToString(DateUtil.addDay(DateUtil.getCurrentMonthFirstDay(), -1),
                                DateStyle.YYYY_MM_DD);
                        break;
                    case "本月":
                        begin = DateUtil.DateToString(DateUtil.getCurrentMonthFirstDay(), DateStyle.YYYY_MM_DD);
                        end = DateUtil.DateToString(DateUtil.addDay(new Date(), -1), DateStyle.YYYY_MM_DD);
                        break;
                }
            }
